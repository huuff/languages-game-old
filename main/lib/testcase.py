...
class SimpleTestCase(TestCase):
    def run(self, base_command, root, config):
        command = OneShotCommand(base_command).set_dir(root).add_arg(self.input)
...
class ListTestCase(TestCase):
    ...
    def run(self, base_command, root, config):
        command = OneShotCommand(base_command).set_dir(root)
...
class FuncTestCase(TestCase):
    def run(self, base_command, root, config):
        command = LongRunningCommand(base_command).set_dir(root)
